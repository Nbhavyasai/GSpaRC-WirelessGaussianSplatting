...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.signal_real, P, 128);
	obtain(chunk, geom.signal_imag, P, 128);
...
__global__ void duplicateWithKeys(
	...
	const float* depths,
	const uint32_t* offsets,  // Updated to accept uint32_t* instead of const int*
	...
	)
{
	...
	FORWARD::preprocess(
		...
		geomState.depths,
		geomState.cov3D,
		geomState.signal_real,
		geomState.signal_imag,
	...
	duplicateWithKeys<<<(P + 255) / 256, 256>>>(
		...
		geomState.means2D,
		geomState.depths,
		geomState.point_offsets ,  // Use point_offsets_32 here
	...
	const float* feature_ptr_real  = geomState.signal_real;
	const float* feature_ptr_imag = geomState.signal_imag;
	...
	float* dpy_dt)
{
	...
	const float* signal_real_ptr =  geomState.signal_real;
	const float* signal_imag_ptr =  geomState.signal_imag;
	...
	CHECK_CUDA(BACKWARD::render(
		...
		dL_dsigreal,
		dL_dsigimag,
		dL_demission_mlps), debug);

	// Take care of the rest of preprocessing. Was the precomputed covariance
	// given to us or a scales/rot pair? If precomputed, pass that. If not,
	// use the one we computed ourselves.
	const float* cov3D_ptr = geomState.cov3D;
	CHECK_CUDA(BACKWARD::preprocess(P, width, height,
		(float3*)means3D,
		emission_mlps,
		radii,
		geomState.clamped,
